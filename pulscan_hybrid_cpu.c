...
typedef struct {
    ...
    int index;
    int z;
} gpu_candidate_struct;

typedef struct {
    float power;
    int index;
} gpu_short_candidate_struct;
...
// decouple chunk width and block width
__global__ void boxcarFilterArray(float* magnitudeSquaredArray, candidate_struct* globalCandidateArray, int chunkWidth, int zMax, int numZs, int numMagnitudes){
    __shared__ float lookupArray[chunkWidth + zmax];
    __shared__ float sumArray[chunkWidth];
    __shared__ gpu_short_candidate_struct searchArray[chunkWidth];
    __shared__ gpu_candidate_struct localCandidateArray[numZs + 1];

    // each threadblock processes 1 chunk of the spectrum, so blockDim.x should be equal to or less than chunkWidth to avoid useless threads
    
    // populate the lookup array
    int globalStartingIndex = chunkWidth * blockIdx.x + threadIdx.x;
    for (int i = 0; i < (chunkWidth+zMax)/blockDim.x; i++){
        if (threadIdx.x + i* blockDim.x < chunkWidth + zmax){
            lookupArray[threadIdx.x + i* blockDim.x] = magnitudeSquaredArray[globalStartingIndex + i* blockDim.x];
        }
    }

    for (int i = 0; i < chunkWidth/blockDim.x; i++){
        if (threadIdx.i + i*blockDim.x < chunkWidth){
            sumArray[threadIdx.x + i* blockDim.x] = lookupArray[threadIdx.x + i* blockDim.x];
            searchArray[threadIdx.x + i* blockDim.x].power = lookupArray[threadIdx.x + i* blockDim.x];
            searchArray[threadIdx.x + i* blockDim.x].index = globalStartingIndex + i* blockDim.x;
        }
    }
    ...
    for (int z = 0; z < zMax; z++){
        if (z == targetZ){
            // max reduction on searchArray
            for (stride = blockDim.x/2; stride > 0; stride /= 2){
                if (threadIdx.x < stride){
                    if (searchArray[threadIdx.x].power < searchArray[threadIdx.x + stride].power){
                        searchArray[threadIdx.x] = searchArray[threadIdx.x + stride];
                    }
                }
            }
            if (threadIdx.x == 0){
                localCandidateArray[targetZ].power = searchArray[0];
                localCandidateArray[targetZ].index = globalStartingIndex;
            }
            outputCounter++;
            if (targetZ == 0){
                targetZ = 1;
        ...
        for (int i = 0; i < (chunkWidth+ blockDim.x - 1)/blockDim.x; i++){
            if (threadIdx.i + i*blockDim.x < chunkWidth){
                sumArray[threadIdx.x + i* blockDim.x] += lookupArray[threadIdx.x + i* blockDim.x+z];
                searchArray[threadIdx.x + i* blockDim.x].power = sumArray[threadIdx.x + i* blockDim.x];
                searchArray[threadIdx.x + i* blockDim.x].index = globalStartingIndex + i* blockDim.x;
            }
        }
    }    
...
void recursive_boxcar_filter_cache_optimised(float* input_magnitudes_array, int magnitudes_array_length, \
    ...
    float* magnitudes_array_GPU;
    cudaMalloc(&magnitudes_array_GPU, sizeof(float) * magnitudes_array_length);
    cudaMemcpy(magnitudes_array_GPU, magnitudes_array, sizeof(float) * magnitudes_array_length, cudaMemcpyHostToDevice);

    // allocate memory on GPU for candidates array
    candidate* candidates_GPU;
    cudaMalloc(&candidates_GPU, sizeof(candidate) * num_blocks * zmax);
